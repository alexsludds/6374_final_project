...
#set_input_delay 100 -clock clk_input [get_ports -regexp a_overwrite\[\d+\]\[\d+\]\[\d+\] -regexp #s_out_overwrite\[\d+\]\[\d+\]\[\d+\] -regexp b_overwrite\[\d+\]\[\d+\]\[\d+\] "shift_direction #command_to_execute array_ack"]
#set_output_delay 100 -clock clk_input [get_ports "ready A_array B_array s_out_array"] 

########### SHADOW REGISTER
create_clock -name clk_input -period 5000 [ get_ports "clk" ]
set_input_delay 100 -clock clk_input [get_ports -regexp parallelIn\[\d+\]\[\d+\]\[\d+\] "loadEN"]
set_output_delay 100 -clock clk_input [get_ports -regexp parallelOut\[\d+\]\[\d+\]\[\d+\]] 
